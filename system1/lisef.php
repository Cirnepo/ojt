...
<nav class="navbar">
    ...
    <a href="lisefCS.php"><button>Computer Science</button></a>
    <a href="lisefIT.php"><button>Information Technology</button></a>
    </header>

        <!-- Search Form -->
        <form action="lisef.php" method="post" class="d-flex mb-4">
            <div class="me-3">
                <label>Year:</label>
                <select name="year">
            ...
            <div class="card-body">
                ...
                <table class="table table-striped">
                    ...
                    <?php
                        ...
                        $query = "
                            SELECT title, yearr, author, pdate, pstat, fdef, fstat, hb, hbdate, file_name 
                            FROM cs 
                            WHERE 1=1
                        ";
                        ...
                        // For the second SELECT (same structure)
                        if ($year) {
                            $types .= 's'; // Year parameter again for the second SELECT
                            $params[] = $year;
                        }

                        if ($search) {
                            for ($i = 0; $i < 9; $i++) {
                                $types .= 's'; // Type for search term again for the second SELECT
                                $params[] = $searchTerm;
                            }
                        }
