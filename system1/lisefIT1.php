            ...
            <div class="card-body">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        $query = "SELECT id, title, yearr, author, fdef, fstat FROM it1 WHERE 1=1";
                        ...
                        if ($search) {
                            $query .= " AND (title LIKE ? OR author LIKE ? OR fdef LIKE ? OR fstat LIKE ?)";
                            $searchTerm = "%" . $search . "%";
                        ...
                        $stmt->bind_result($id, $title, $yearr, $author, $fdef, $fstat);
                        $foundResults = false;
                        ...
                        while ($stmt->fetch()) {
                            ...
                            echo "<td style='background-color: #f69697; color: black;'>" . htmlspecialchars($fdef) . "</td>";
                            echo "<td style='background-color: #f69697; color: black;'>";
                            // For Status Option
                            ...
                            echo "<option value='Passed'" . ($fstat == "Passed" ? " selected" : "") . ">Passed</option>";
                            echo "<option value='Failed'" . ($fstat == "Failed" ? " selected" : "") . ">Failed</option>";
                            echo "</select>";
                            echo "</form>";

                            echo "<td>";
                            echo "<button type='button' class='btn btn-outline-primary' onclick=\"if (confirm('Are you sure you want to proceed?')) { window.location.href='move_to_it2.php?id=" . htmlspecialchars($id) . "&title=" . urlencode($title) . "&yearr=" . urlencode($yearr) . "&author=" . urlencode($author) . "&pdate=" . urlencode($fdef) . "&pstat=" . urlencode($fstat) . "'; }\">Save</button>";
                            echo "</td>";
                        }
