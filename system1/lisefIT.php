            ...
            <div class="card-body">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        $query = "SELECT id, title, yearr, author, pdate, pstat FROM it WHERE 1=1";
                        ...
                        if ($search) {
                            $query .= " AND (title LIKE ? OR author LIKE ? OR pdate LIKE ? OR pstat LIKE ?)";
                            $searchTerm = "%" . $search . "%";
                        ...
                        $stmt->bind_result($id, $title, $yearr, $author, $pdate, $pstat);
                        $foundResults = false;
                        ...
                        while ($stmt->fetch()) {
                            ...
                            echo "<option value='Passed'" . ($pstat == "Passed" ? " selected" : "") . ">Passed</option>";
                            echo "<option value='Failed'" . ($pstat == "Failed" ? " selected" : "") . ">Failed</option>";
                            echo "</select>";
                            echo "</form>";
                            echo "<td>";
                            echo "<button type='button' class='btn btn-outline-primary' onclick=\"if (confirm('Are you sure you want to proceed?')) { window.location.href='move_to_it1.php?id=" . htmlspecialchars($id) . "&title=" . urlencode($title) . "&yearr=" . urlencode($yearr) . "&author=" . urlencode($author) . "&pdate=" . urlencode($pdate) . "&pstat=" . urlencode($pstat) . "'; }\">Save</button>";
                            echo "</td>";
